...
suffix = '.csv'
path = ''
file_name = 'training_20min_avg_volume_1_entry'
...
subplot(1,1,1)
plot_date(df['time_window'], (df['volume']-df['volume2'])/df['volume'], color=colors[0], linewidth=1.0, linestyle="-",label="volume")
plot_date(df['time_window'], (df['has_etc']-df['has_etc2'])/df['has_etc'], color=colors[1], linewidth=1.0, linestyle="-",label="has_etc")
