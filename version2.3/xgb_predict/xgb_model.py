...
def xgbmodel(train_file,test_file):
    ...
    train = training_df.iloc[:,:label_offset].values
    labels = training_df.iloc[:,label_offset:].values
  
    test = test_df.iloc[:,:label_offset].values
    ...
    #'lambda':450,  # L2 正则项权重
    'subsample':0.4, # 采样训练数据比例，设置为0.5，随机选择一般的数据实例 (0:1]
    'colsample_bytree':0.7, # 构建树时用的特征采样比率 (0:1]
    #'min_child_weight':12, # 节点的最少特征数
    'silent':1 ,
    'eta': 0.005, # 每次迭代完成后更新权重时的步长，越小训练越慢，如同学习率
    'seed':710,
    'nthread':4,
    }
    ...
    offset = len(training_df[training_df.index<'2016-10-15 00:00:00'])   # 划分验证集从2016-10-XX 00:00:00
    num_rounds = 10000
